...
import 'package:check/pages/home/home_page.dart';
...
class _LoginPageState extends State<LoginPage> {
  ...
  Color _loginBtnColor = kPrimaryColor;
  bool _isLoad = false;
  ...
  Future<void> _login() async {
    if (_userEmailTEC.text.isNotEmpty && _userPasswordTEC.text.isNotEmpty) {
      ...
      if (responce.statusCode == 201) {
        token = body["token"];
        currentUserName = body["user"]["name"];
        currentUserAvatar = body["user"]["avatar"] ?? toNull;
        currentUserAge = body["user"]["age"];
        currentUserAddress = body["user"]["address"];
        currentUserIsPremium = body["user"]["is_premium"];
        currentUserEmail = body["user"]["email"] ?? "Оруулаагүй байна";
        currentUserLikedAdvice = body["user"]["liked_advice_id"] ?? "";
        currentUserReportedAdvice = body["user"]["reported_advice_id"] ?? "";
